...
export function getTopLevelReadableElementsOnPage(): HTMLElement[] {
  // Helper function to check if an element is in the ignore list
  const isIgnored = (element: HTMLElement): boolean => {
    return IGNORE_LIST.includes(element.tagName);
  };

  // Helper function to check if an element has text content
  const hasTextContent = (element: HTMLElement): boolean => {
    return (
      element.textContent !== null && element.textContent.trim().length > 0
    );
  };

  // Helper function to recursively check for top-level readability
  const isTopLevelReadable = (element: HTMLElement): boolean => {
    if (isIgnored(element) || !hasTextContent(element)) {
      return false;
    }
  ...
  const topLevelReadableElements: HTMLElement[] = [];

  allElements.forEach((element) => {
    if (isTopLevelReadable(element as HTMLElement)) {
      topLevelReadableElements.push(element as HTMLElement);
    }
  });
